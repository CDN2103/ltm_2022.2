    ...
    while (1) 
    {
        ...
        if (fds[0].revents & POLLIN) 
        {
            ...
            if (userNumber < MAX_USER)
            {
                ...
                // Send message
                char *message = "Enter (client_id: client_name)\n";
            ...
            if (fds[i].revents & (POLLIN | POLLERR))
            {
                ...
                if (j == userNumber)
                {
                    ...
                    if (ret == 2)
                    {
                        ...
                        if (strcmp("client_id:", cmd) == 0)
                        {
                            ...
                            userNumber++;
                            char *message = "chat private: PRIVATE_TO_(clientid) message\n";
                            send(client, message, strlen(message), 0);
                            continue;
                        }
                    }
                ...
                else
                {
                    ...
                    if (strncmp("PRIVATE_TO_", buff, 11) == 0)
                    {
                        ...
                        {
                            // Wrong format
                            char *message = "format chat private: PRIVATE_TO_(clientid) message\n";
                            send(client, message, strlen(message), 0);
                            continue;
                        }
